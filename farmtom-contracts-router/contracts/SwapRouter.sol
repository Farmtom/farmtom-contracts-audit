...
contract SwapRouter is ISwapRouter02 {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'ensure: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, '_addLiquidity: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = SwapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, '_addLiquidity: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = ILPPair(pair).burn(to);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            ILPPair(SwapLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'swapExactTokensForTokens: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'swapTokensForExactTokens: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'swapExactETHForTokens: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'swapTokensForExactETH: INVALID_PATH');
        amounts = SwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'swapTokensForExactETH: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = SwapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'swapExactTokensForETH: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, address(this));
    ...
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'swapETHForExactTokens: INVALID_PATH');
        amounts = SwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'swapETHForExactTokens: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(SwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = SwapLibrary.sortTokens(input, output);
            ILPPair pair = ILPPair(SwapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'swapExactTokensForTokensSupportingFeeOnTransferTokens: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    {
        require(path[0] == WETH, 'swapExactETHForTokensSupportingFeeOnTransferTokens: INVALID_PATH');
        ...
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'swapExactETHForTokensSupportingFeeOnTransferTokens: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    {
        require(path[path.length - 1] == WETH, 'swapExactTokensForETHSupportingFeeOnTransferTokens: INVALID_PATH');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SwapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(WETH).balanceOf(address(this));
        require(amountOut >= amountOutMin, 'swapExactTokensForETHSupportingFeeOnTransferTokens: INSUFFICIENT_OUTPUT_AMOUNT');
