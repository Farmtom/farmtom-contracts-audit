...
*/
contract FarmtomToken is ERC20("Farmtom Token", "FRTM") {
    ...
    constructor() public{
        ...
        // After initializing the token with the original constructor of ERC20
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmountRate < 10000 && maxTransferAmount() > 0) {
            if (
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        ...
        require(_maxTransferAmountRate >= BASE_MAX_TRANSFER_AMOUNT_RATE, "updateMaxTransferAmountRate: _maxTransferAmountRate should be at least _maxTransferAmountRate");
    ...
    modifier onlyMinters() {
        require(isOwner() || isSecondaryToken(), "caller is not the minter");
        _;
    }
    ...
    /// @dev Returns true if the caller is secondary contracts.
    function isSecondaryToken() internal view returns (bool) {
        return msg.sender == address(secondaryToken);
    }

    /// @dev Sets the secondary token address.
    function setupSecondaryToken(address _secondary) external onlyOperator{
        require(address(secondaryToken) == address(0), "The secondary token address has already been set up. No one can change it anymore.");
        secondaryToken = _secondary;
    }
    ...
    */
    function mint(address _to, uint256 _amount) external virtual override onlyMinters  {
        _mint(_to, _amount);
    }
